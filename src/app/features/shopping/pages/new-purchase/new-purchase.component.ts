...
export class NewPurchaseComponent implements OnInit {
      ...
      next: (response: IOrderArrayResponse) => {
        if (response.isSuccess && response.value) {
          ...
          this.orders.forEach(order => {
            if (order.orderDetails && !order.totalAmount) {
              order.totalAmount = this.calculateOrderTotal(order);
            }
          });
  ...
  private calculateOrderTotal(order: IOrder): number {
    ...
    return order.orderDetails.reduce((total, item) => total + (item.priceBuy * item.quantity), 0);
  }
  ...
  get paginatedProducts() {
    ...
    return this.productosAgregados.slice(startIndex, startIndex + this.modalItemsPerPage);
  }
  ...
  get modalTotalPages() {
    return Math.ceil(this.productosAgregados.length / this.modalItemsPerPage);
  }
  ...
  openEditOrderModal(order: IOrder) {
    ...
    this.purchaseService.getOrderById(order.idOrder).subscribe({
      next: (response: IOrderResponse) => {
        if (response.isSuccess && response.value) {
          const orderDetails = response.value.orderDetails || [];
          this.productosAgregados = orderDetails.map(detail => ({
            idProduct: detail.idProduct,
            nameProduct: this.getProductById(detail.idProduct)?.nameProduct || 'Producto desconocido',
  ...
  openStatusModal(order: IOrder) {
    ...
    this.remainingChars = 75;
    this.showStatusModal = true;
  }

  updateDescription(event: any) {
    this.remainingChars = 75 - event.target.value.length;
  }
  ...
  addProduct(): void {
    ...
    this.selectedProductId = null;
    this.quantity = 1;
    this.modalCurrentPage = Math.ceil(this.productosAgregados.length / this.modalItemsPerPage);
    this.cdr.detectChanges();
  }

  removeProduct(index: number): void {
    const globalIndex = (this.modalCurrentPage - 1) * this.modalItemsPerPage + index;
    this.productosAgregados.splice(globalIndex, 1);
    
    if (this.paginatedProducts.length === 0 && this.modalCurrentPage > 1) {
  ...
  updateQuantity(index: number, newQuantity: number) {
    const globalIndex = (this.modalCurrentPage - 1) * this.modalItemsPerPage + index;
    if (newQuantity > 0 && globalIndex < this.productosAgregados.length) {
      this.productosAgregados[globalIndex].quantity = newQuantity;
      this.cdr.detectChanges();
    }
  }
  ...
  updateOrderStatus(): void {
    ...
    if (this.newStatus === 'C' && (!this.statusDescription || this.statusDescription.trim() === '')) {
      Swal.fire('Error', 'Debe proporcionar una descripción para cancelar la orden', 'error');
      return;
    }

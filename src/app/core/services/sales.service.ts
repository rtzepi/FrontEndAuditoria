...
export class SalesService {
  ...
  getProducts(categoryId?: number): Observable<IProductResponse> {
    const url = categoryId 
      ? `${this.apiUrl}/Inventory/List?id=${categoryId}`
      : `${this.apiUrl}/Inventory/List`;
    return this.http.get<IProductResponse>(url).pipe(
      catchError(this.handleError)
    );
  }

  getProductsByName(name: string): Observable<IProductResponse> {
    return this.http.get<IProductResponse>(`${this.apiUrl}/Inventory/ListByName?name=${name}`).pipe(
      catchError(this.handleError)
    );
  }
